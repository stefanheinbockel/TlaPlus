Protecting the switches from changing when one of the corresponding semaphores showed "GO" produced another deadlock.
The reason was that we never reset the semaphores once a train had passed. 
This model adds this property by changing the state of the semaphore in the action for the train movement.
It also adds the check that a switch most not be changed while a train is currently on it, which would also be possible otherwise.
In reality, both checks are done by employing track occupancy detectors and axle counters that can detect when a 
train enters or leaves a certain section of the track.

Finally, this model resolves the deadlock and all crashes, but it still has problems with stuttering - steps which repeat each 
other without the trains moving their destinations. The next model will resolve these.
---------------------------- MODULE 07_TrainSiding_ResettingSemaphores ----------------------------
